#!/usr/bin/env python3
"""Process Mahjong Ways 1 xxb capture files into URL replay data."""
...
DEFAULT_GAME_URL = (
    "https://fish-games.s3.amazonaws.com/MahjongWays1/index.html"
...
WILD_SYMBOL = 0
SCATTER_SYMBOL = 1
HIDDEN_INDEXES = {0, 5, 6, 11, 12, 17, 18, 23, 24, 29}
...
def is_mahjong1_title(title: str | None) -> bool:
    normalized = "".join((title or "").lower().split()).replace("-", "").replace("_", "")
    return any(alias in normalized for alias in MAHJONG1_TITLE_ALIASES)
...
def _game_url_base(token: str | None = None) -> str:
    game_url = os.environ.get("MAHJONG1_GAME_URL_BASE", DEFAULT_GAME_URL)
...
def _load_json_lines(source: Path) -> tuple[list[dict[str, Any]], int]:
    ...
    for line_no, line in enumerate(source.read_text(encoding="utf-8").splitlines(), 1):
        ...
        try:
            ...
            skipped += 1
            print(f"[process_mahjong1] skip line {line_no}: invalid JSON")
            continue
        if not isinstance(obj, dict):
            skipped += 1
            print(f"[process_mahjong1] skip line {line_no}: JSON root is not an object")
            continue
        rows.append(obj)
    return rows, skipped
...
def _record_has_win(record: dict[str, Any]) -> bool:
    return (
        _float_value(record.get("iswin")) == 1
        or bool(_wp_positions(record))
        or bool(record.get("lw"))
    )
...
def _validate_record_shape(
    ...
    if not _record_has_win(record) and possible_wins:
...
def _majiang_error_checks_for_records(
    ...
    if records:
        ...
        if _record_has_win(last_record):
            issues.append(_majiang_issue(
                file_name,
                replay_start,
                github_path,
                ...
                f"bet文件最后一条仍是中奖继续状态iswin={last_record.get('iswin')}，但当前文件没有下一条数据",
            ))
    return issues
...
def _bet_amount(records: list[dict[str, Any]]) -> float:
    if not records:
        return 0.0
    first = records[0]
    return _float_value(first.get("tbb"))
...
def _summarize_replay_file(
    ...
    has_continue_state = any(record.get("st") in (4, 22) for record in records) or len(records) > 1
...
def process_txt(
    ...
    output_dir = script_dir / "mahjong1date" / batch
    print(f"create directory: mahjong1date/{batch}/")

    summary = split_capture(txt_path, output_dir, mode=mode)
    github_path = f"mahjong1date/{batch}"
    ...
    print(
        ...
        f"skipped={summary.skipped_lines} output=mahjong1date/{batch}/"
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Process Mahjong1 xxb capture txt/zip for URL replay.")
    parser.add_argument("file", nargs="?", help="Source .txt or .zip file. Defaults to update/mahjong1*.")
...
def main() -> int:
    ...
    if not file_path:
        ...
        files = [
            ...
            if item.name.lower().startswith("mahjong1") and item.suffix.lower() in {".txt", ".zip"}
        ]
        if not files:
            print("No mahjong1 txt/zip file found in update/.")
            return 0
